...
class _PlacePickScreenState extends State<PlacePickScreen>
  ...
  int _selectedIndex = 0; // ✅ 카테고리 인덱스 추적
  ...
  Future<void> _initSessionAndLoad() async {
    try {
      ...
      final sid =
      (jsonDecode(res.body) as Map<String, dynamic>)['selectionId'] as String;
      _selectionId = sid;
  ...
  Future<void> _loadTab(int i) async {
    ...
    try {
      ...
      if ((opt.presetKey ?? '').isNotEmpty) {
        final res =
        await ApiClient.get('/api/custom/places/preset', query: {
          'key': opt.presetKey!,
          'page': 1,
          'limit': 20,
        });
      ...
      _results[i] =
          items.map((e) => NearbyPlace.fromJson(e)).toList().cast<NearbyPlace>();
      setState(() {});
    } catch (e) {
      _results[i] = [];
      setState(() {});
  ...
  Future<void> _remove(NearbyPlace p) async {
    ...
    try {
      ...
      setState(() => _picked.removeWhere(
              (e) => (e.contentId ?? e.title) == (p.contentId ?? p.title)));
      _fitMap();
    } catch (e) {
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text('제거 실패: $e')));
    }
  }
  ...
  Future<void> _preview() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text('코스 생성 실패: $e')));
    }
  }
        ...
        children: [
          ...
          SizedBox(
            ...
            child: GoogleMap(
              ...
              const CameraPosition(target: _jeju, zoom: 10.8),
                ...
                ..._picked.asMap().entries.map((e) {
                  ...
                  return Marker(
                    ...
                    infoWindow: InfoWindow(
                        title: '$idx. ${p.title}', snippet: p.addr),
                  );
                }),
              },
